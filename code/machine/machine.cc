...
void Machine::loadPageFrame(int vpn, int ppn)
{
    ...
    pt[ppn].valid = true;
#ifdef FIFO_REPLACE
    updateFIFOFlag(pt, ppn, NumPhysPages);
#endif
#else
    pt[vpn].ppn = ppn;
    pt[vpn].valid = true;
...
#ifdef FIFO_REPLACE
    updateFIFOFlag(pt, vpn, pageTableSize);
#endif
#endif
...
#endif
    if(debug->IsEnabled('a')) cerr<<"Read addr: "<<kernel->currentThread->space->getCurrentNoffHeader().code.inFileAddr+vpn*PageSize<<" from the file into addr: "<<ppn*PageSize<<" in the main memory!"<<endl;

    kernel->currentThread->space->getCurrentOpenFile()->ReadAt(&(kernel->machine->mainMemory[ppn*PageSize]), PageSize, kernel->currentThread->space->getCurrentNoffHeader().code.inFileAddr+vpn*PageSize);
}