...
void Machine::showTLB()
{
    ...
    {
        cerr<<tlb[i].vpn<<"\t"<<tlb[i].ppn<<"\t"<<tlb[i].valid<<"\t"<<tlb[i].readOnly<<"\t"<<tlb[i].use<<"\t"<<tlb[i].dirty<<"\t"<<tlb[i].FIFOFlag<<"\t"<<tlb[i].LRUFlag<<endl;
    }
}
...
int Machine::findOneToReplace(TranslationEntry* t)
{
	int targetv,targeti=0,len = sizeof(*t)/sizeof(t[0]);
    targetv = len+1;
    for(int i=0;i<len;++i)
    {
#ifdef FIFO_REPLACE
        if(t[i].valid&&targetv>t[i].FIFOFlag)
        {
...
void Machine::updateFIFOFlag(TranslationEntry* t, int pos)
{
    int len = sizeof(*t)/sizeof(t[0]),num=0;
    for(int i=0;i<len;++i)
    {
        if(t[i].valid&&i!=pos)
        {
...
void Machine::updateLRUFlag(TranslationEntry* t, int pos)
{
	int len = sizeof(*t)/sizeof(t[0]),num=0;
    for(int i=0;i<len;++i)
    {
...
void Machine::loadPageFrame(unsigned int vpn, unsigned int ppn)
{
    bool readFromVM = false;
#ifdef USE_RPT
    pt[ppn].tid = kernel->currentThread->getTID();
    pt[ppn].vpn = vpn;
    if(pt[ppn].dirty) readFromVM = true;
#ifdef FIFO_REPLACE
    updateFIFOFlag(pt, ppn);
#endif
#else
    pt[vpn].ppn = ppn;
    if(pt[vpn].dirty) readFromVM = true;
#ifdef FIFO_REPLACE
    updateFIFOFlag(pt, vpn);
#endif
#endif
    kernel->machine->mmBitmap->Mark(ppn);
    if(!readFromVM)
    {
        kernel->currentThread->getCurrentOpenFile()->ReadAt(&(kernel->machine->mainMemory[ppn*PageSize]), PageSize, kernel->currentThread->getCurrentNoffHeader().code.inFileAddr+vpn*PageSize);
    }
    else
    {
        char* tmpFileName1;
        ...
        exec->ReadAt(&(kernel->machine->mainMemory[ppn*PageSize]), PageSize, vpn*PageSize);
        delete exec;
    }
}