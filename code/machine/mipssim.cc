...
void Machine::OneInstruction(Instruction *instr)
{
	...
	if (debug->IsEnabled('m'))
	{
		...
		sprintf(buf, str->format, TypeToReg(str->args[0], instr), TypeToReg(str->args[1], instr), TypeToReg(str->args[2], instr));
		cout << "\t" << buf << "\n";
	}
	...
	cerr<<"instruction operation code is:"<<(int)instr->opCode<<endl;
	switch (instr->opCode)
	{
